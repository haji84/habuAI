...
def fit_model(root,data,cfg):
    ...
    feats=[c for c in base+[c for c in train if c.startswith("bio_") and (c.endswith("_count") or c.endswith("_nearest_m"))] if c in train]
    X=train[feats].replace([np.inf,-np.inf],np.nan); y=train.habu_capture.astype(int)
