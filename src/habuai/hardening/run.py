...
from .reconstruct import build_reconstructed_spatial_learning,reconstruct_historical_routes
from .roads import combine_roads,recover_supplemental_roads
from .spatial_backtest import run_reconstructed_spatial_backtest
from .spatial_history import add_historical_spatial_features
...
def apply_hardening(pipeline):
    ...
    def hardened_run(root):
        ...
        if not reconstructed_spatial.empty:
            reconstructed_spatial=add_static_context(reconstructed_spatial,segs,root)
            historical_spatial_backtest_static=run_reconstructed_spatial_backtest(root,reconstructed_spatial,cfg,feature_mode="static")
            reconstructed_spatial,historical_spatial_feature_audit=add_historical_spatial_features(reconstructed_spatial,events,segs,cfg,root=root)
            reconstructed_spatial.to_csv(paths.processed/"reconstructed_spatial_learning_2026-05_07.csv",index=False)
            historical_spatial_backtest_enhanced=run_reconstructed_spatial_backtest(root,reconstructed_spatial,cfg,feature_mode="enhanced")
        else:
            historical_spatial_backtest_static={"status":"empty"};historical_spatial_backtest_enhanced={"status":"empty"};historical_spatial_feature_audit={"status":"empty"}
        ...
        env_cols=["moon_age_days","moon_illumination","moon_phase_sin","moon_phase_cos","fog_wmo_flag","temp_dewpoint_spread_c","fog_proxy_flag","temperature_change_3visits_c","tide_height_cm","tide_change_1h_cm","tide_state_code","minutes_to_nearest_turning_tide","tide_source_available"];qa["environmental_feature_missing_rate"]={c:(None if c not in model_data else float(model_data[c].isna().mean())) for c in env_cols};qa["feature_sources"]={"lunar":"deterministic synodic calculation","fog":"Open-Meteo WMO code 45/48 plus temperature-dewpoint proxy","temperature":"Open-Meteo hourly","tide":"JMA Amami O9 hourly predicted tide table; local authoritative tide_hourly.csv overrides when supplied"};qa["learning_sources"]={"gpx_visit_rows":int((model_data.learning_row_source=="gpx_visit").sum()),"capture_gps_anchor_rows":int((model_data.learning_row_source=="capture_gps_anchor").sum()),"reconstructed_spatial_rows":int(len(reconstructed_spatial)),"reconstructed_spatial_positive_rows":int(reconstructed_spatial.spatial_label.sum()) if not reconstructed_spatial.empty else 0,"reconstructed_spatial_negative_rows":int((reconstructed_spatial.spatial_label==0).sum()) if not reconstructed_spatial.empty else 0,"note":"B-high/B-medium reconstruction is spatial/location evidence only; no fabricated historical time enters temporal model"};qa["model_roles"]={"evaluation":{"artifact":"models/habu_occurrence_evaluation.joblib","cutoff":cfg["baseline_cutoff"],"positives":metrics.get("positives")},"production":{"artifact":"models/habu_occurrence_production.joblib","alias":"models/habu_occurrence.joblib","positives":production.get("positives"),"uses_all_current_learning_rows":True}};qa["walk_forward_backtest"]=backtest;qa["historical_route_reconstruction"]=reconstruction;qa["reconstructed_spatial_learning"]=spatial_learning_audit;qa["historical_spatial_feature_audit"]=historical_spatial_feature_audit;qa["historical_spatial_backtest_static"]=historical_spatial_backtest_static;qa["historical_spatial_backtest_enhanced"]=historical_spatial_backtest_enhanced
        (paths.reports/"qa_summary.json").write_text(json.dumps(qa,ensure_ascii=False,indent=2),encoding="utf-8");return {"segments":len(segs),"supplemental_segments":len(supplemental),"gpx_points":len(points),"visits":len(visits),"events":len(events),"duplicates_removed":removed,"canonical_master":canonical_audit,"positive_label_audit":qa["positive_label_audit"],"learning_sources":qa["learning_sources"],"historical_route_reconstruction":reconstruction,"reconstructed_route_rows":len(reconstructed_routes),"reconstructed_spatial_learning":spatial_learning_audit,"historical_spatial_feature_audit":historical_spatial_feature_audit,"historical_spatial_backtest_static":historical_spatial_backtest_static,"historical_spatial_backtest_enhanced":historical_spatial_backtest_enhanced,"metrics":metrics,"production_metrics":production,"holdout":hold,"strict_holdout":strict,"backtest":backtest,"qa":qa,"forecast":forecast}
