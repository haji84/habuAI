...
def apply_hardening(pipeline):
    ...
    def hardened_run(root):
        ...
        weather=pipeline.fetch_weather(root,weather_events,visits,cfg)

        # Exposure rows remain GPX-only.  This preserves the meaning of zero-data and
        # visit denominators while the model can additionally learn from capture GPS anchors.
        data=join_weather(visits,weather);data=add_outcomes_bio(data,events,segs,cfg);data=add_static_context(data,segs,root);data=pipeline.add_exposure_features(data);data=add_lunar_features(data);data=add_weather_derived_features(data);data=join_optional_tide_features(data,root)
        data["learning_row_source"]="gpx_visit"

        anchor_base=build_capture_anchor_visits(events,data)
        anchors=pd.DataFrame()
        if not anchor_base.empty:
            anchors=join_weather(anchor_base,weather)
            anchors=add_outcomes_bio(anchors,events,segs,cfg)
            anchors["habu_capture"]=1
            anchors["habu_individuals"]=pd.to_numeric(anchors["anchor_individual_count"],errors="coerce").fillna(1).astype(int)
            anchors=add_static_context(anchors,segs,root)
            anchors=pipeline.add_exposure_features(anchors)
            anchors=apply_anchor_prior_visits(anchors,data)
            anchors=add_lunar_features(anchors);anchors=add_weather_derived_features(anchors);anchors=join_optional_tide_features(anchors,root)
            anchors["learning_row_source"]="capture_gps_anchor"

        model_data=pd.concat([data,anchors],ignore_index=True,sort=False) if not anchors.empty else data.copy()
        model_data=model_data.sort_values("entered_at",kind="stable").reset_index(drop=True)
        model_data.to_csv(paths.processed/"learning_10m_road.csv",index=False);model_data.to_parquet(paths.processed/"learning_10m_road.parquet",index=False);mp.to_csv(paths.processed/"gpx_points_matched.csv",index=False);visits.to_csv(paths.processed/"segment_visits.csv",index=False)
        ...
        label_audit=build_positive_label_audit(events,model_data);label_audit.to_csv(paths.reports/"positive_label_audit.csv",index=False)
        label_ok=int((label_audit.audit_status=="ok").sum()) if not label_audit.empty else 0
        road_matched_capture_events=int(((events.species=="ハブ")&(events.event_type=="捕獲")&events.segment_id.notna()).sum())
        if label_ok!=road_matched_capture_events:
            raise RuntimeError(f"GPS capture learning gate failed: road_matched={road_matched_capture_events}, labeled_ok={label_ok}")

        metrics=fit_model(root,model_data,cfg);hold=score_holdout(root,model_data,cfg);strict=strict_holdout_score(events);(paths.reports/"strict_holdout_2026-08-28.json").write_text(json.dumps(strict,ensure_ascii=False,indent=2),encoding="utf-8");cut=pd.Timestamp(cfg["baseline_cutoff"]);forecast=pipeline.make_forecast(root,data[data.entered_at<cut].copy(),cfg);(paths.reports/"model_metrics.json").write_text(json.dumps(metrics,ensure_ascii=False,indent=2),encoding="utf-8");(paths.reports/"latest_score.json").write_text(json.dumps(hold,ensure_ascii=False,indent=2),encoding="utf-8");write_map(root,segs,data,events,points,cfg);qa=write_qa(root,pipeline,cfg,points,raw,events,model_data,removed,strict)
        qa["canonical_master"]=canonical_audit;qa["positive_label_audit"]={"capture_events":int(len(label_audit)),"road_matched_capture_events":road_matched_capture_events,"labeled_ok":label_ok,"not_labeled":int(len(label_audit)-label_ok),"status_counts":label_audit.audit_status.value_counts(dropna=False).to_dict() if not label_audit.empty else {},"gpx_positive_rows":int(((model_data.habu_capture==1)&(model_data.learning_row_source=="gpx_visit")).sum()),"gps_anchor_positive_rows":int(((model_data.habu_capture==1)&(model_data.learning_row_source=="capture_gps_anchor")).sum())}
        qa["foundation_steps"]["2_full_capture_master_road_match_and_label_audit"]="complete" if canonical_audit==EXPECTED_CANONICAL and label_ok==road_matched_capture_events else "partial"
        ...
        qa["environmental_feature_missing_rate"]={c:(None if c not in model_data else float(model_data[c].isna().mean())) for c in env_cols}
        ...
        qa["learning_sources"]={"gpx_visit_rows":int((model_data.learning_row_source=="gpx_visit").sum()),"capture_gps_anchor_rows":int((model_data.learning_row_source=="capture_gps_anchor").sum()),"note":"capture anchors are model evidence only and do not count as GPX exploration visits"}
        (paths.reports/"qa_summary.json").write_text(json.dumps(qa,ensure_ascii=False,indent=2),encoding="utf-8")
        return {"segments":len(segs),"supplemental_segments":len(supplemental),"gpx_points":len(points),"visits":len(visits),"events":len(events),"duplicates_removed":removed,"canonical_master":canonical_audit,"positive_label_audit":qa["positive_label_audit"],"learning_sources":qa["learning_sources"],"metrics":metrics,"holdout":hold,"strict_holdout":strict,"qa":qa,"forecast":forecast}
