...
def _night_date(date_value, timestamp):
    if pd.notna(timestamp):
        return (timestamp - pd.Timedelta(hours=7)).date().isoformat()
    day = pd.to_datetime(str(date_value), errors="coerce")
    return day.date().isoformat() if pd.notna(day) else None
...
def _canonical_row(r: dict, event_type: str) -> dict:
    timestamp = _parse_clock(r.get("日付"), r.get("時刻"))
    count = pd.to_numeric(r.get("数"), errors="coerce")
    return {
        "canonical_id": str(r.get("ID")),
        "timestamp": timestamp,
        "session_start": pd.NaT,
        "event_type": event_type,
        "species": str(r.get("種別")),
        "individual_count": int(count) if pd.notna(count) else (0 if event_type == "no_capture" else 1),
        "size": _size_code(r.get("サイズ")),
        "wetness": None,
        "lat": pd.to_numeric(r.get("緯度"), errors="coerce"),
        "lon": pd.to_numeric(r.get("経度"), errors="coerce"),
        "night_date": _night_date(r.get("日付"), timestamp),
        "moon_age_observed": pd.to_numeric(r.get("月齢"), errors="coerce"),
        "tide_observed": None if pd.isna(r.get("潮")) else str(r.get("潮")),
        "tide_direction_observed": None if pd.isna(r.get("上げ下げ")) else str(r.get("上げ下げ")),
        "weather_observed": None if pd.isna(r.get("天候")) else str(r.get("天候")),
        "temperature_observed_c": pd.to_numeric(r.get("気温"), errors="coerce"),
        "canonical_source": None if pd.isna(r.get("ソース")) else str(r.get("ソース")),
        "raw_text": f"canonical:{r.get('ID')} {event_type}",
    }
...
def load_canonical_capture_master(root: Path) -> pd.DataFrame:
    ...
    rows = [_canonical_row(r, "捕獲") for r in src.to_dict("records") if r.get("イベント") == "捕獲"]
    return pd.DataFrame(rows)


def load_canonical_auxiliary_events(root: Path) -> pd.DataFrame:
    path = root / "data" / "canonical" / "habu_auxiliary_events.csv"
    if not path.exists():
        return pd.DataFrame()
    ...
    for r in src.to_dict("records"):
        ...
        if event_type in {"roadkill_sighting", "sighting", "no_capture", "轢死", "目撃"}:
            rows.append(_canonical_row(r, event_type))
    return pd.DataFrame(rows)


def load_canonical_habu_events(root: Path) -> pd.DataFrame:
    capture = load_canonical_capture_master(root)
    aux = load_canonical_auxiliary_events(root)
    parts = [x for x in (capture, aux) if not x.empty]
...
def merge_canonical_capture_master(raw_events, canonical):
    ...
    cols = sorted(set(raw_events.columns) | set(canonical.columns))
    return pd.concat([raw_events.reindex(columns=cols), canonical.reindex(columns=cols)], ignore_index=True)
    ...
    for r in h.itertuples():
        ...
        if pd.isna(getattr(r, "lat", np.nan)) or pd.isna(getattr(r, "lon", np.nan)):
            ...
            if candidates.empty:
                ...
                reason = "" if positives else "no_visit_within_10_minutes"
        rows.append({
