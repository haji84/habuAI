...
def _as_jst(ts: pd.Series) -> pd.Series:
    return pd.to_datetime(ts,format="mixed",utc=True).dt.tz_convert("Asia/Tokyo")


def _survey_night(ts: pd.Series, rollover_hour: int) -> pd.Series:
    return (_as_jst(ts)-pd.Timedelta(hours=rollover_hour)).dt.date.astype(str)
...
def run_walk_forward_backtest(root: Path,model_data: pd.DataFrame,events: pd.DataFrame,cfg: dict)->dict:
    """Leakage-safe nightly replay using a 07:00 future-information wall."""
    ...
    data["entered_at"]=_as_jst(data.entered_at);events=events.copy();events["timestamp"]=_as_jst(events.timestamp);gpx=data[data.learning_row_source=="gpx_visit"].copy()
    if gpx.empty:
    ...
    for night in sorted(gpx.survey_night.unique()):
        ...
        if prior_night_rows:
            expected_effort=float(np.median(prior_night_rows));historical_rate=(sum(prior_night_captures)+count_prior_events)/(sum(prior_night_rows)+count_prior_exposure);pre_count=float(historical_rate*expected_effort);pre_round=int(np.rint(pre_count));exposure_count=float(historical_rate*len(test));exposure_round=int(np.rint(exposure_count))
        else:
            expected_effort=np.nan;historical_rate=np.nan;pre_count=np.nan;pre_round=np.nan;exposure_count=np.nan;exposure_round=np.nan
