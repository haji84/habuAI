...
from .reconstruct_network import rescue_with_road_network
...
def _ordered_session_segments(visits: pd.DataFrame) -> dict[str,list[str]]:
    out={}
    for session,df in visits.sort_values(["session_file","visit_index"]).groupby("session_file",sort=True):out[str(session)]=df.segment_id.astype(str).tolist()
    return out
...
def _union_graph(sessions:dict[str,list[str]]):
    ...
    for (a,b),support in edge_support.items():
        cost=1.0/max(support,1);adj[a][b]=min(adj[a].get(b,float("inf")),cost);adj[b][a]=min(adj[b].get(a,float("inf")),cost)
...
def _shortest_path(adj,start,goal):
    ...
    while q:
        ...
        for nxt,w in adj.get(node,{}).items():
            nc=cost+w
            if nc<best.get(nxt,float("inf")):best[nxt]=nc;prev[nxt]=node;heapq.heappush(q,(nc,nxt))
...
def _rescue_with_union_graph(cap_segments:set[str],sessions:dict[str,list[str]],min_anchor_coverage:float):
    adj,node_support,edge_support=_union_graph(sessions);present=[s for s in cap_segments if s in node_support];coverage=len(present)/len(cap_segments) if cap_segments else 0.0
    ...
    for target in sorted((s for s in present if s!=root),key=lambda s:(-node_support[s],s)):
        ...
        used_edges.extend(tuple(sorted((a,b))) for a,b in zip(path,path[1:]))
        for seg in path:
            if seg not in ordered:ordered.append(seg)
    edge_supports=[edge_support[e] for e in used_edges] or [node_support[root]]
    return {"piece":ordered,"coverage":coverage,"node_support":node_support,"support_sessions":max(node_support[s] for s in present),"median_edge_support":float(np.median(edge_supports)),"min_edge_support":int(min(edge_supports))}


def reconstruct_historical_routes(root:Path,events:pd.DataFrame,visits:pd.DataFrame,cfg:dict,segs=None)->tuple[pd.DataFrame,dict]:
    """Reconstruct May-July spatial exposure without fabricating historical traversal time."""
    ...
    for night,ndf in hist.groupby("reconstruction_night",sort=True):
        matched=ndf[ndf.segment_id.notna()].copy();ordered_caps=matched.sort_values("timestamp").segment_id.astype(str).tolist();cap_segments=set(ordered_caps);total_caps=int(len(ndf));matched_caps=int(len(matched))
        ...
        union_rescue=_rescue_with_union_graph(cap_segments,sessions,rescue_cov) if (not primary or original_confidence=="C-low") else None
        road_rescue=None
        if union_rescue is None and (not primary or original_confidence=="C-low") and segs is not None:road_rescue=rescue_with_road_network(ordered_caps,sessions,segs,cfg)
        rescue_meta={}
        if union_rescue:
            piece=union_rescue["piece"];cov=union_rescue["coverage"];support_sessions=union_rescue["support_sessions"];confidence="B-high" if union_rescue["median_edge_support"]>=rescue_high_edge else "B-medium";session="MULTI_GPX_UNION";a=b=-1;support=union_rescue["node_support"];source="multi_gpx_union_graph_rescue";rescue_status="rescued-from-"+(original_confidence or "unreconstructed");rescue_meta={"rescue_median_edge_support":union_rescue["median_edge_support"],"rescue_min_edge_support":union_rescue["min_edge_support"]}
        elif road_rescue:
            piece=road_rescue["piece"];cov=1.0;support_sessions=road_rescue["support_sessions"];confidence=road_rescue["confidence"];session="ROAD_NETWORK_GPX_PRIOR";a=b=-1;support=road_rescue["support"];source="road_network_gpx_prior_rescue";rescue_status="road-network-rescued-from-"+(original_confidence or "unreconstructed");rescue_meta={"road_rescue_detour_ratio":road_rescue["detour_ratio"],"road_rescue_supported_fraction":road_rescue["supported_fraction"],"road_rescue_median_support":road_rescue["median_support"],"road_rescue_path_m":road_rescue["weighted_path_m"],"road_rescue_shortest_path_m":road_rescue["baseline_shortest_path_m"]}
        elif primary:
        ...
        for order,seg in enumerate(piece):rows.append({"night":night,"route_order":order,"segment_id":seg,"reconstruction_source":source,"template_session_file":session,"template_span_start_visit":a,"template_span_end_visit":b,"capture_anchor_segment":seg in cap_set,"capture_coverage":cov,"support_sessions":support_sessions,"segment_support_sessions":support.get(seg,0),"segment_support_fraction":support.get(seg,0)/max(len(sessions),1),"reconstruction_confidence":confidence,"rescue_status":rescue_status,"historical_time_reconstructed":False})
        item={"night":night,"capture_events":total_caps,"matched_capture_events":matched_caps,"unique_capture_segments":len(cap_segments),"status":"reconstructed","best_template":session,"best_coverage":cov,"candidate_sessions":support_sessions,"reconstructed_segment_rows":len(piece),"confidence":confidence,"original_confidence":original_confidence,"rescue_status":rescue_status};item.update(rescue_meta);audit.append(item)
    out=pd.DataFrame(rows);report={"status":"ok","method":"capture GPS -> later GPX single template -> multi-GPX adjacency rescue -> constrained road-network rescue weighted toward repeatedly explored later-GPX roads","nights_total":int(hist.reconstruction_night.nunique()),"capture_events":int(len(hist)),"road_matched_capture_events":int(hist.segment_id.notna().sum()),"nights_reconstructed":int(sum(a.get("status")=="reconstructed" for a in audit)),"nights_coverage_100pct":int(sum(a.get("status")=="reconstructed" and a.get("best_coverage",0)>=1.0 for a in audit)),"nights_coverage_ge_50pct":int(sum(a.get("status")=="reconstructed" and a.get("best_coverage",0)>=0.5 for a in audit)),"confidence_counts":dict(Counter(a.get("confidence") for a in audit if a.get("confidence"))),"rescued_nights":int(sum("rescued" in str(a.get("rescue_status","")) and a.get("rescue_status")!="rescue-failed" for a in audit)),"road_network_rescued_nights":int(sum(str(a.get("rescue_status","")).startswith("road-network-rescued") for a in audit)),"remaining_c_low_nights":int(sum(a.get("confidence")=="C-low" for a in audit)),"nightly":audit,"warning":"Spatial exposure only. Later GPX timestamps are never copied into May-July; network rescue is accepted only under configured detour constraints."}
...
def build_reconstructed_spatial_learning(root:Path,reconstructed:pd.DataFrame)->tuple[pd.DataFrame,dict]:
    ...
    agg=trusted.groupby(["night","segment_id"],as_index=False).agg(spatial_label=("spatial_label","max"),reconstruction_confidence=("reconstruction_confidence",lambda s:"B-high" if (s=="B-high").any() else "B-medium"),sample_weight=("sample_weight","max"),segment_support_fraction=("segment_support_fraction","max"),support_sessions=("support_sessions","max"),template_session_file=("template_session_file","first"),reconstruction_source=("reconstruction_source","first"));agg["learning_row_source"]="historical_reconstructed_spatial_exposure";agg["historical_time_reconstructed"]=False;agg.to_csv(out_path,index=False)
    report={"status":"ok","rows":int(len(agg)),"nights":int(agg.night.nunique()),"positive_segment_rows":int(agg.spatial_label.sum()),"negative_segment_rows":int((agg.spatial_label==0).sum()),"confidence_counts":agg.reconstruction_confidence.value_counts().to_dict(),"rescued_rows":int(agg.reconstruction_source.isin(["multi_gpx_union_graph_rescue","road_network_gpx_prior_rescue"]).sum()),"rescued_nights":int(agg.loc[agg.reconstruction_source.isin(["multi_gpx_union_graph_rescue","road_network_gpx_prior_rescue"]),"night"].nunique()),"usage":"spatial/location model only; excluded from temporal/weather/time-of-night model","remaining_c_low_excluded":int((reconstructed.reconstruction_confidence=="C-low").sum())};report_path.write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8");return agg,report
